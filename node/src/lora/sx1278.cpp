...
int joinOTAA(LoRaWANNode& node) {
    ...
    log("LoRaWAN OTAA activation successful.", "LORA", Serial);
}
int joinABP(LoRaWANNode& node) {
    ...
    log("LoRaWAN ABP activation successful.", "LORA", Serial);
}
...
void LORA::begin() {
    ...
    debug(state != RADIOLIB_ERR_NONE, F("beginOTAA failed"), state, true);
    
    state = node.activateOTAA();
    debug(state != RADIOLIB_ERR_NONE, F("OTAA activation failed"), state, true);
